...
// 获取用户投票状态
func VoteStautusPost(userID int64, postid int64) (value int64, err error) {
	zap.L().Debug("VoteForPost",
		zap.Int64("userID", userID),
		zap.Int64("postID", postid))

	value, err = redis.VoteStatusPost(strconv.Itoa(int(userID)), strconv.Itoa(int(postid)))
	if err != nil {
		zap.L().Error("VoteForPost Failed", zap.Error(err))
		return value, err
	}
